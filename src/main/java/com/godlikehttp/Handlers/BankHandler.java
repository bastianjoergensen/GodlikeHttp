...
@Slf4j
public class BankHandler extends BaseHttpHandler
{
    private Item[] bankState;
    ...
    public Object getData()
    {
        ...
        jsonObject.add("items", RuneLiteAPI.GSON.toJsonTree(bankState));
        return jsonObject;
    }
    ...
    {
        if (event.getItemContainer() == GodlikeHttpPlugin.getClient().getItemContainer(InventoryID.BANK))
        {
            ItemManager itemManager = GodlikeHttpPlugin.getItemManager();

            bankState = event.getItemContainer().getItems();
            itemCount = 0;
            gePrice = 0;
            
            for (Item i : bankState)
            {
